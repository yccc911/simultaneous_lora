...
class Transformer(torch.nn.Module):
    def __init__(self, layer_id: int, args: LLMModelArgs):
        ...
        # feed forward
        self.w1_gate: Linear = None  # also gate FNN * dim
    ...
    def init_lora_layer_weight(self,
        ...
        linear_layer_list = [self.wk_, self.wq_, self.wv_, self.wo_, self.w1_gate, self.w2_down, self.w3_up]
        linear_layer_name_list = ["k_proj", "q_proj", "v_proj", "o_proj", "w1_gate_proj", "w2_down_proj", "w3_up_proj"]
    ...
    def forward(self,
        ...
        w1 = self.w1_gate.forward(score_norm_data, input_args)
        w3 = self.w3_up.forward(score_norm_data, input_args)

        data = data + self.w2_down.forward(F.silu(w1) * w3, input_args)
...
# same function as implemented in /transformers/models/llama/modeling_llama.py apply_rotary_pos_emb()
...
class LlamaModel(LLMModel):
    ...
    def from_pretrained(path: str,
        ...
        logging.info(f"Initializing {llama_args.n_layers_} hidden layers")
        for idx, layer in enumerate(llama_model.model.layers):
            ...
            model.layers_[idx].w1_gate = Linear(layer.mlp.gate_proj, device=device)
            model.layers_[idx].w2_down = Linear(layer.mlp.down_proj, device=device)
            model.layers_[idx].w3_up = Linear(layer.mlp.up_proj, device=device)
        ...
        for transformer_layer in self.layers_:
            for lora_config in config["lora"]:
                ...
                lora_layer_list = [transformer_layer.wq_.loras_, transformer_layer.wk_.loras_,
                                    ...
                                    transformer_layer.w1_gate.loras_, transformer_layer.w2_down.loras_,
                                    transformer_layer.w3_up.loras_]
    ...
    def get_lora_weight_dict(self, lora_name: str) -> Tuple[Dict[str, torch.Tensor], List[str]]:
        ...
        for idx, transformer_layer in enumerate(self.layers_):
            ...
            lora_layer_list = [transformer_layer.wq_, transformer_layer.wk_,
                                ...
                                transformer_layer.w1_gate, transformer_layer.w2_down,
                                transformer_layer.w3_up]
            lora_layer_name_list = ["q_proj", "k_proj", "v_proj", "o_proj", "w1_gate_proj", "w2_down_proj", "w3_up_proj"]
